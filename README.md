...
#### Cosine Similarity
- 수면장애 진료인원과 각 변수 간 지역별 분포의 유사도 측정
- 벡터 정규화 후 코사인 유사도(cosine similarity)를 활용해 비교
- 높은 유사도일수록 지역별 분포 패턴이 수면장애와 유사하다고 해석
...
- 가장 높은 유사도를 보인 변수는 **야식 섭취**와 **야간 교통량**
- 활동량은 낮은 유사도를 보여 설명 변수로서의 해석력은 제한적
